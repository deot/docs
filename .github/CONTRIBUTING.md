...
- **`npm run cli:build`**：本仓库 README 演示使用根 `index.html` 作为 workspace，不适合作为发布路径（会把 monorepo 源码一并复制）。日常预览请用 `npm run cli:dev` 或 `npm run cli:preview`；直接部署时托管根 `index.html`。静态托管若没有 HTML history fallback，请同时托管 [`404.html`](../404.html)：它把当前 URL 写入 `sessionStorage['@deot/docs:redirect']`，由 Client 还原深链。

## 文档

- 用户向说明见根 [README.md](../README.md) 与各 `packages/*/README.md`。
- 本仓库演示入口是根 [`index.html`](../index.html)（README 聚合），结构与根 README「快速开始」中的通用 `site/` 模板不同。深链回退见 [`404.html`](../404.html) 与 Client 的 `@deot/docs:redirect` 约定。
