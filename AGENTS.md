...
- `npm run cli:build` is not the publish path for this repository's README demo: a root workspace build would copy monorepo sources into `dist`. Use `npm run cli:dev` or `npm run cli:preview`, or deploy the root `index.html` directly. Static hosts without HTML history fallback should also deploy `404.html`, which stores the current URL in `sessionStorage['@deot/docs:redirect']` for the client to restore.
