...
const start = (
	...
	const options = normalizeOptions(config.prefetch);
...
/**
 * 封装浏览器空闲预加载入口；每次 start 的运行状态均保留在独立闭包中。
 */
class IdlePrefetchScheduler {
	 ...
	 * @returns 关闭时返回 null，否则返回完整配置。
	 */
	normalizeOptions(value: DocsConfig['prefetch']) {
	 ...
	 * 启动一轮可取消的空闲预加载会话。
	 * @param config 当前站点配置。
	 * @param target 注册 idle、timer 和 online 的窗口。
	 * @returns 停止并取消当前会话的函数。
	 */
	start(config: DocsConfig, target: IdleWindow = window) {
		return start(config, target);
	}
}
