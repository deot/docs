...
export const createDocsDemo = (lang: string): RendererDocument => {
	const overview = copy(
		...
		[
			...
			'',
			':::tip',
			'开发模式从 Header 进入编辑器后，保存会把内联文档写成 `.page.json`，并回写 `routes[\'/\'].content`。',
			':::',
			'',
			':::warning',
		...
		[
			...
			'',
			':::tip',
			'In development, saving from the header editor writes an inline document to `.page.json` and updates `routes[\'/\'].content`.',
			':::',
			'',
			':::warning',
